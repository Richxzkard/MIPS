...
find_local_minima_label: .asciiz "\nTesting local minimum points: 1, 3, 2, 4, 6, 5, 7, 8\n"
find_local_minima_lbl: .asciiz "Expected output:\n0, 0, 1, 0, 0, 1, 0, 0\nObtained output:\n"
...
la $a0, new_line
syscall
la $a0, find_local_minima_label
syscall

la $a0, local_minima_input
la $a1, local_minima_output
la $s0, local_minima_input_size
lw $a2, 0($s0)
jal find_local_minima

# Print output
li $v0, 4
la $a0, find_local_minima_lbl
syscall

la $t1, local_minima_output
la $s0, local_minima_input_size
li $t2, 0
lw $t3, 0($s0)
print_loop:
